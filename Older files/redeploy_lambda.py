...
def redeploy_lambda():
    """Redeploy the Lambda function with minimal changes to force a fresh deployment"""
    
    # Lambda function name
    FUNCTION_NAME = 'SevaAI-S3Agent'
    
    print(f"Redeploying Lambda function: {FUNCTION_NAME}")
...
def list_buckets():
    ...
    try:
        ...
        print(f"Error listing buckets: {type(e).__name__}: {str(e)}")
...
def list_objects(bucket):
    ...
    try:
        ...
        print(f"Found {len(objects) if response.get('Contents') else 0} objects")
        ...
        print(f"Error listing objects: {type(e).__name__}: {str(e)}")
        return f"❌ S3 Error: {str(e)}"
...
'''
    ...
    try:
        ...
        print("The Lambda function has been redeployed with additional logging.")
        print("Check CloudWatch logs for detailed error messages.")
        
        return True
        
    except Exception as e:
        print(f"❌ Error redeploying Lambda function: {str(e)}")
        return False
