...
def deploy_lambda():
    """Deploy the Lambda function with history support"""
    
    # Lambda function name
    FUNCTION_NAME = 'SevaAI-S3Agent'
    
    print(f"Deploying Lambda function with history support: {FUNCTION_NAME}")
    
    # Create a zip file in memory
    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, 'a', zipfile.ZIP_DEFLATED) as zip_file:
        # Add the Lambda function code
        lambda_file = 'lambda_with_history.py'
    ...
    try:
        ...
        print("\n✅ Deployment complete!")
        print("You can now use the updated S3 agent interface to interact with your S3 buckets.")
