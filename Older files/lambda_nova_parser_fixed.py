...
def lambda_handler(event, context):
    """Lambda handler that uses Nova Micro to parse user input and execute AWS commands"""
    ...
    try:
        ...
        ai_model = body.get('ai_model', 'nova')  # Default to Nova Micro
        
        print(f"Processing request: {user_message} (session: {session_id}, model: {ai_model})")
        ...
        # Use Nova Micro to parse the user's request
        parsed_intent = parse_with_nova(user_message)
        
        # Check if Nova needs more information
        ...
        # Execute the command based on Nova's parsing
...
def parse_with_nova(user_message):
    """Use Nova Micro to parse user input into structured commands"""
    try:
...
"""
        ...
        # Fixed Nova Micro invocation - removed max_tokens parameter
...
def test_connectivity():
    ...
    try:
        # Test Bedrock with fixed invocation
