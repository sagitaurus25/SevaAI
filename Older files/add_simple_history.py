...
def deploy_simple_history():
    """Deploy a simple history function that doesn't rely on DynamoDB"""
    
    # Lambda function name
    FUNCTION_NAME = 'SevaAI-S3Agent'
    
    print(f"Deploying simple history function for: {FUNCTION_NAME}")
...
# In-memory history storage (will be reset when Lambda cold starts)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if user_message_lower == 'list files':
            response = "Which bucket would you like to list files from?"
            store_conversation(session_id, user_message, response)
            return create_response(response)
        ...
        if user_message_lower.startswith('create bucket '):
            ...
            if bucket_name:
                ...
                response = "Please specify a bucket name. Example: create bucket my-new-bucket"
                store_conversation(session_id, user_message, response)
                return create_response(response)
        ...
        if user_message_lower.startswith('delete bucket '):
            ...
            if bucket_name:
                ...
                response = "Please specify a bucket name. Example: delete bucket my-bucket"
                store_conversation(session_id, user_message, response)
                return create_response(response)
        ...
        response = "I can help with S3 operations. Try 'list buckets', 'list files', or 'help'."
        store_conversation(session_id, user_message, response)
        return create_response(response)
...
def store_conversation(session_id, user_message, bot_response):
    ...
    # Limit history size
    if len(conversation_history[session_id]) > MAX_HISTORY_ITEMS:
        conversation_history[session_id] = conversation_history[session_id][-MAX_HISTORY_ITEMS:]
...
def get_conversation_history(session_id):
    ...
    for i, item in enumerate(history):
        ...
        summary += f"{timestamp} - You: {user_msg}\\n"
        summary += f"{timestamp} - Bot: {bot_msg}\\n\\n"
...
'''
    ...
    try:
        ...
        print("\n✅ Deployment complete!")
        print("The Lambda function now includes a simple in-memory history function.")
