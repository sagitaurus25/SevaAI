...
def seed_knowledge_base(table):
    ...
    s3_commands = [
        ...
        {
            "intent_pattern": "upload file",
            "service": "s3",
            "action": "upload_file",
            "required_params": ["bucket", "object_key", "local_file"],
            "example_phrases": [
                "put file in bucket", 
                "upload to s3", 
            ...
            "needs_followup": True,
            "followup_question": "Which local file would you like to upload and to which bucket?",
            "syntax_template": "Upload {local_file} to {bucket} as {object_key}"
        },
        {
            "intent_pattern": "download file",
            "service": "s3",
            ...
            "example_phrases": [
                ...
                "download from s3", 
                "save file from bucket"
            ],
            "needs_followup": True,
            "followup_question": "Which file would you like to download, from which bucket, and where should I save it locally?",
            "syntax_template": "Download {object_key} from {bucket} to {local_file}"
        }
    ]
