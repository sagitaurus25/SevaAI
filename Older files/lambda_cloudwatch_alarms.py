...
def get_cloudwatch_alarms():
    ...
    try:
        response = cloudwatch_client.describe_alarms(MaxRecords=50)
        ...
        for alarm in alarms:
            ...
            alarm_list.append(f"{name}: {state}")
        
        return {
            'alarms': alarm_list,
            'count': len(alarm_list),
            'message': f'Found {len(alarm_list)} CloudWatch alarms'
        }
        
    except Exception as e:
        return {'error': f'Failed to get CloudWatch alarms: {str(e)}'}
