...
def test_connection():
    """Test connection to the DynamoDB table"""
    try:
        table = dynamodb.Table(TABLE_NAME)
        response = table.scan(Limit=1)
        print(f"✅ Successfully connected to DynamoDB table '{TABLE_NAME}'")
        print(f"✅ Table contains {len(response.get('Items', []))} items")
        return True
...
def main():
    parser = argparse.ArgumentParser(description='Test the S3 command knowledge base in AWS')
    ...
    parser.add_argument('--test', '-t', action='store_true', help='Test connection to DynamoDB')
