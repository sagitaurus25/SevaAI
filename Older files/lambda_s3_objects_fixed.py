...
def lambda_handler(event, context):
    ...
    try:
        ...
        if 'objects' in request or 'files' in request:
            bucket_name = extract_bucket_name(request)
            return list_objects(bucket_name)
        elif 'list' in request and 'bucket' in request and 'objects' not in request:
            return list_buckets()
        elif 'move' in request:
            return move_object(event.get('request', ''))
            ...
            return {
                ...
                'supported_operations': [
                    ...
                    'Move object source-bucket/file.ext to dest-bucket',
...
def move_object(request):
    ...
    try:
        # Handle "Move object path/file.ext to bucket" pattern
        if 'object' in request and ' to ' in request:
            parts = request.split(' to ')
            if len(parts) == 2:
                object_part = parts[0].replace('Move object ', '').strip()
                dest_bucket = parts[1].strip()
                
                # Extract source bucket and object key from path
                if '/' in object_part:
                    # Assume first part before / is bucket, rest is object key
                    path_parts = object_part.split('/', 1)
                    source_bucket = path_parts[0]
                    object_key = path_parts[1]
                else:
                    return {'error': 'Please specify source bucket: "Move object source-bucket/file.ext to dest-bucket"'}
        ...
        # Handle "from bucket1 to bucket2" pattern (original)
        words = request.split()
        ...
        if not from_idx or not to_idx:
            return {'error': 'Use format: "Move object source-bucket/file.ext to dest-bucket"'}
