    ...
    try:
        ...
        if service == 'unknown':
            return create_response(200, {
                ...
                'supported_services': ['S3', 'EC2', 'Lambda', 'IAM', 'CloudWatch'],
                'examples': {
                    ...
                    'CloudWatch': ['How many times my Lambda functions ran today', 'Show EC2 CPU usage']
                }
            })
...
def determine_service(request: str) -> str:
    ...
    # CloudWatch/Metrics indicators
    if any(word in request_lower for word in ['ran', 'invocation', 'metrics', 'usage', 'cpu']):
        return 'cloudwatch'
    ...
    # EC2-specific terms (including VPC)
    if any(word in request_lower for word in ['security group', 'key pair', 'start', 'stop', 'reboot', 'vpc']):
        return 'ec2'
