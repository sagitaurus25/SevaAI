...
def lambda_handler(event, context):
    """Universal AWS Router with Nova Micro NLP - Nova handles ALL requests first"""
    ...
    try:
        ...
        # Send ALL requests to Nova first - no keyword filtering
        parsed_intent = parse_with_nova(user_request)
        
        if parsed_intent.get('error'):
            # Only fallback to keywords if Nova completely fails
            return handle_fallback(user_request, body)
        
        # Handle based on Nova's understanding
        service = parsed_intent.get('service', 'unknown')
        action = parsed_intent.get('action', '')
        
        # Handle help requests identified by Nova
        if action == 'help' or service == 'help':
            result = handle_help_request(user_request, parsed_intent)
            return create_response(200, {
        ...
        # Generic response for unrecognized requests
...
def parse_with_nova(user_request):
    """Use Nova Micro to parse ALL user requests"""
    try:
...
{{
  "service": "s3|ec2|lambda|iam|cloudwatch|help|unknown",
  "action": "list|upload|start|stop|show|move|explain|help",
  "parameters": {{"bucket": "name", "instance": "id", "file": "name"}},
  "confidence": 0.9
}}

IMPORTANT: ANY question about help, services, capabilities, or "what can you do" should return service: "help", action: "help"

Service detection rules:
- Questions about help/services/capabilities → service: "help", action: "help"
- "upload", "bucket", "s3" → service: "s3"
- "instances", "ec2", "start", "stop" → service: "ec2"
- "lambda", "functions" → service: "lambda"
- "users", "roles", "iam" → service: "iam"
- "metrics", "cloudwatch", "alarms" → service: "cloudwatch"

Examples:
"How can you help me with AWS services" → {{"service": "help", "action": "help", "parameters": {{}}}}
"What services do you support" → {{"service": "help", "action": "help", "parameters": {{}}}}
"What can you do" → {{"service": "help", "action": "help", "parameters": {{}}}}
...
def handle_fallback(user_request, body):
    """Fallback only when Nova completely fails"""
    return create_response(200, {
        'request': user_request,
        'service': 'fallback',
        'result': {
            ...
            'note': 'AI parsing temporarily unavailable, using fallback mode'
        },
        'ai_parsed': False
    })

def handle_help_request(user_request, parsed_intent=None):
    """Handle help requests identified by Nova"""
    ...
    for service_key, service_info in service_help.items():
        if service_key in request_lower:
            return {
    ...
    return {
        ...
        'powered_by': 'Nova Micro AI for natural language understanding'
    }
