...
def restore_working_lambda():
    """Restore the Lambda function to its working state"""
    
    # Lambda function name
    FUNCTION_NAME = 'SevaAI-S3Agent'
    
    print(f"Restoring Lambda function to working state: {FUNCTION_NAME}")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if 'delete file' in user_message_lower and 'from' in user_message_lower:
            parts = user_message.split('delete file')[1].split('from')
            if len(parts) == 2:
...
'''
    ...
    try:
        ...
        print("The Lambda function has been restored to its working state.")
