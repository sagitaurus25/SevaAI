...
def lambda_handler(event, context):
    """Simple Lambda handler for S3 operations"""
    try:
        ...
        if user_message.lower().startswith('create bucket'):
            parts = user_message.lower().split('create bucket')
            if len(parts) > 1:
                bucket = parts[1].strip()
                result = create_bucket(bucket)
                return create_response(result)
        
        # For other commands, use Nova to parse
