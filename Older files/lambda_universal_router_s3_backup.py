...
def lambda_handler(event, context):
    """Universal AWS Router - Hybrid approach with keyword backup"""
    ...
    try:
        ...
        # Quick keyword check for S3 operations (backup)
        s3_keywords = ['bucket', 's3', 'upload', 'objects', 'move']
        if any(keyword in user_request.lower() for keyword in s3_keywords):
...
Return JSON:
{{
  "service": "ec2|lambda|iam|cloudwatch|unknown",
  "action": "list|start|stop|show|explain",
  "parameters": {{"instance": "id"}}
}}

Examples:
"What are my EC2 instances" → {{"service": "ec2", "action": "list", "parameters": {{}}}}
"Show Lambda functions" → {{"service": "lambda", "action": "list", "parameters": {{}}}}"""
